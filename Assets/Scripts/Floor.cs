...
public class Floor : MonoBehaviour
{
    ...
    public void initFloor(int id, Vector3[] baseVertices, float floorHeight, Vector3 position, float windowDistance, float doorDistance, Material material) {
        this.floorNumber = id;
        ...
        for (int i = 0; i < numWalls; i++) {
            ...
            generateWall(i, mesh, newPosition, windowDistance, doorDistance, material);
        }

    }

    // Generate a wall based on "wall specific arguments"
    private void generateWall(int id, Mesh wallMesh, Vector3 wallPosition, float windowDistance, float doorDistance, Material wallMaterial) {
        GameObject obj = new GameObject($"Wall {id}");
        obj.transform.parent = transform; // Set this building as parent
        obj.AddComponent<Wall>();
        obj.GetComponent<Wall>().initWall(wallMesh, wallPosition, windowDistance, doorDistance, wallMaterial);
    }
