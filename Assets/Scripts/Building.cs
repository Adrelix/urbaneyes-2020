...
public class Building : MonoBehaviour {
    ...
    public void initBuilding(BuildingData data, float floorHeight, float windowDistance) {
        ...
        generateFloor(0, floorHeight * 1.25f, windowDistance, firstFloorMaterial);
        ...
        for (int i = 1 ; i < data.levels; i++) {
            generateFloor(i, floorHeight, windowDistance, upperFloorsMaterial);
        }
    ...
    private void generateFloor(int id, float currentFloorHeight, float windowDistance, Material material) {
        GameObject obj = new GameObject($"Floor {id}");
        obj.transform.parent = transform; // Set this building as parent
        obj.AddComponent<Floor>();
        obj.GetComponent<Floor>().initFloor(id, this.floorBase, currentFloorHeight, this.transform.position, windowDistance, material);
